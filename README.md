...
**Multiduino core** — Outback Electronics provides a custom Arduino core specifically designed for this board. It includes tailored libraries for every on-board peripheral that expose hardware-specific functionality not possible with generic libraries. The Multiduino core is recommended for any project making full use of the board's hardware. See the [Multiduino Core repository](https://github.com/cowboydaniel/outback-arduino-avr) for installation instructions.
...
1. Install the [Arduino IDE](https://www.arduino.cc/en/software) if not already installed.
2. Follow the installation instructions in the [Multiduino Core repository](https://github.com/cowboydaniel/outback-arduino-avr) to add the board to the IDE.
